...
# Compute expected values (averages)
rho_bar = mean(rho)
varrho_bar = mean(varrho)
C = varrho_bar / rho_bar

# 
delta_vec = C * colMeans(theta) - colMeans(vartheta)
...
results_list <- pblapply(t_max_values, function(time){
  print(time)
  simulation_log_equi(
    time = time)
})
...
save(results_list, file = paste0(dir$output, "results_list_DGP_log0119.RData"))
#save(results_list, file = paste0(dir$output, "results_list0331.RData"))

load(paste0(dir$output, "results_list_DGP_log0402_20.RData"))
...
simulation_B_list_DGP_equi <- function(time, B, eta_Z, eta_X, alg = FALSE) {
  ...
  for (S in seq(1, n)) {
    ...
    # Compute ratio of treated vs control for F and Y
    ratio_F <- EF1 / EF0
    ratio_Y <- EY1 / EY0
    
    # Calculate scaling factor to make the ratios equal
    scaling_factor <- ratio_F / ratio_Y
    
    # Apply the multiplicative adjustment to Y[1, ]
    Y[1, ] <- Y[1, ] * scaling_factor
...
simulation_log_equi <- function(time, alg = FALSE) {
  ...
  # Compute expected values (averages)
  rho_bar = mean(rho[1:time])
  varrho_bar = mean(varrho)
  C = varrho_bar / rho_bar
  
  # 
  delta_vec = C * colMeans(theta[1:time, ]) - colMeans(vartheta)
  vartheta_local = sweep(vartheta, 2, delta_vec, "+")
  # Potential outcomes
